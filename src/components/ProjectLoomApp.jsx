...
import { Plus, Users, MessageCircle, Settings, Play, Pause, Send, Upload, Trash2, Eye, EyeOff, Download, Loader2, Key, User, LogOut, X } from 'lucide-react';
...
const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const savedUsers = localStorage.getItem('loom-users');
    const currentSession = localStorage.getItem('loom-current-user');
  ...
  useEffect(() => {
    if (users.length > 0) {
      localStorage.setItem('loom-users', JSON.stringify(users));
    }
  }, [users]);

  // Save current session to localStorage whenever user changes
  useEffect(() => {
    if (user) {
      localStorage.setItem('loom-current-user', JSON.stringify(user));
    } else {
      localStorage.removeItem('loom-current-user');
    }
  }, [user]);
  ...
  const updateUserPersonas = (personas) => {
    updateUser({ personas });
  };

  const updateUserEnvironments = (environments) => {
    updateUser({ environments });
  };

  const updateUserSimulations = (simulations) => {
    updateUser({ simulations });
  };

  const updateUserApiKeys = (apiKeys) => {
    updateUser({ apiKeys });
  };
      ...
      <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-md mx-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-purple-600 text-white py-3 px-6 rounded-lg hover:bg-purple-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors font-semibold"
          >
            {isLoading ? (
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-600 to-blue-600 flex items-center justify-center">
        <div className="text-center text-white max-w-md mx-4">
          <h1 className="text-5xl font-bold mb-4">🔍 Project Loom</h1>
          <p className="text-xl mb-8 text-purple-100">
            AI Simulation Platform - Create and interact with customizable AI-powered personas
          </p>
          ...
          <div className="space-y-4">
            <button
              ...
              className="w-full bg-white text-purple-600 py-3 px-6 rounded-lg hover:bg-gray-100 transition-colors font-semibold"
            >
              Sign In
            </button>
            <button
              ...
              className="w-full bg-purple-700 text-white py-3 px-6 rounded-lg hover:bg-purple-800 transition-colors font-semibold border border-purple-400"
            >
              Create Account
            </button>
          </div>
...
const AuthenticatedApp = () => {
  const { user, logout, updateUserPersonas, updateUserEnvironments, updateUserApiKeys } = useAuth();
  const [currentPage, setCurrentPage] = useState('dashboard');
  const [isSimulationRunning, setIsSimulationRunning] = useState(false);
  const [showApiSettings, setShowApiSettings] = useState(false);

  // Use user's data instead of local state
  const personas = user.personas || [];
  const environments = user.environments || [];
  const apiKeys = user.apiKeys || { openai: '', claude: '' };
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => updateUserApiKeys({ ...apiKeys, openai: e.target.value })}
              placeholder="sk-..."
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => updateUserApiKeys({ ...apiKeys, claude: e.target.value })}
              placeholder="sk-ant-..."
              className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
            <p className="text-xs text-gray-500 mt-1">
    ...
    <nav className="bg-gradient-to-r from-purple-600 to-blue-600 text-white p-4 shadow-lg">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-3 ml-6 pl-6 border-l border-purple-400">
            ...
            <button
              onClick={logout}
              className="px-3 py-2 rounded-lg hover:bg-purple-700 transition-colors flex items-center"
            >
  ...
  const CreatePersona = () => {
    ...
    const handleSubmit = (e) => {
      ...
      updateUserPersonas([...personas, newPersona]);
  ...
  const CreateEnvironment = () => {
    ...
    const handleSubmit = (e) => {
      ...
      updateUserEnvironments([...environments, newEnvironment]);
